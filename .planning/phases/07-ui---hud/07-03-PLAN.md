...
must_haves:
  truths:
    ...
    - "ESC during gameplay opens pause menu, Resume button resumes"
  ...
  key_links:
    ...
    - from: "BerzerkGame.cs"
      to: "PauseMenu"
      via: "OnResume event -> GameState.Playing, ESC opens pause (one-way)"
...
- ESC key: pauses during Playing only (Playing -> Paused). Does NOT unpause — only the Resume button unpauses (Paused -> Playing). ESC does NOT exit game. Exiting is only through Quit buttons.
...
6. HealthBar.Trigger wired to OnDamageTaken
  ...
  <done>
BerzerkGame has complete 5-state game loop. Start menu shows first, transitions to gameplay. ESC pauses (Playing -> Paused), Resume button unpauses (Paused -> Playing). Score tracks enemy kills. Ammo/score/pickups display in HUD. Game over shows final score with buttons. Health bar flashes on damage.
  </done>
</task>
...
8. **Pause menu**: Press ESC during gameplay. Game should freeze, semi-transparent dark overlay appears with "PAUSED" title, "Resume" and "Quit" buttons. Mouse cursor becomes visible. Click Resume to continue (cursor hides, gameplay resumes). Pressing ESC again should NOT unpause — only the Resume button unpauses.
...
- ESC pauses (Playing -> Paused only), Resume button unpauses (no longer exits)
