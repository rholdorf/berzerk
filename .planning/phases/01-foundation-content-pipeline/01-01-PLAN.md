...
must_haves:
  truths:
    - "MonoGame project builds successfully on macOS"
...
```
Build succeeds. Game window opens with cornflower blue background. Close window manually.
  </verify>
  <done>
MonoGame game project builds and runs, showing empty game window. Project uses BerzerkGame class name.
  </done>
</task>
...
dotnet build Berzerk.sln
```
Both projects build successfully. Check that Berzerk.ContentPipeline.dll exists in bin/Debug/net8.0/.
  </verify>
  <done>
...
- [ ] `dotnet run --project Berzerk/Berzerk.csproj` opens game window
